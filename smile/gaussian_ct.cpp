...
/* Box-Muller sampler */
static inline void box_muller(const double sigma)
{
	...
	randombytes(r, BOX_MULLER_BYTES);
...
/* FACCT exp(x) */
static inline __m128d exp_constant(const __m128d x, const __m128d V_SIGMA_INV)
{
	...
	/* evaluate 2^a */
	vsum = _mm_add_sd(_mm_mul_sd(_mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[0])), vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[1])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[2])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[3])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[4])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[5])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[6])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[7])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[8])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[9])));
	vsum = _mm_add_sd(_mm_mul_sd(vsum, vx2), _mm_castsi128_pd(_mm_cvtsi64_si128(EXP_COFF[10])));	
 ...
 * EndIf
 */
int64_t discrete_gaussian(const double center, const double sigma)
{
	unsigned char r[DISCRETE_BYTES]; 	
	const double SIGMA = sigma; /* sigma=2, 32768, 131072 in our benchmark */
	const double DISCRETE_NORMALISATION = 0.5 * M_SQRT1_2 * M_2_SQRTPI * (1.0 / SIGMA); /* 1/S=1/(sigma*sqrt(2*pi)) */
	const double SIGMA_INV = -1.0 / (2 * SIGMA * SIGMA); /* -1/sigma^2 */

	const __m128d V_SIGMA_INV = {SIGMA_INV, 0};
	const __m128d V_DISCRETE_NORMALISATION = {DISCRETE_NORMALISATION, 0};
	...
	_mm_storel_epi64((__m128i *)(&drc), _mm_castpd_si128(_mm_mul_sd(exp_constant(_mm_mul_sd(vc, vc), V_SIGMA_INV), V_DISCRETE_NORMALISATION)));

	randombytes(r, COMP_ENTRY_SIZE);
	dcmprc = -comp(r, drc);
	vcmprc = _mm_loadl_epi64((__m128i *)(&dcmprc));

	while (true)
	{
		randombytes(r, DISCRETE_BYTES);
		...
		for (i = 0; i < 8; i++, head++)
		{
			...
			if (dcmp)
			{
				...
				_mm_storel_epi64((__m128i *)(&drej), _mm_castpd_si128(exp_constant(_mm_mul_sd(_mm_add_sd(vyrc, vy), _mm_sub_sd(vyrc, vy)), V_SIGMA_INV)));
...
for (t = 0; t < T; t++)
{
	...
	for (i = 0; i < TOTAL; i++)
	{
		sample[i] = discrete_gaussian(center[i], SIGMA0);
	}
	cycle2 = cpucycles();
