 ...
 * Name:        poly_acc
 *
 * Description: Sum of NTT coefficients.
 *
 * Arguments:   - int64_t *r: pointer to output array
 *              - const poly *a: pointer to first input polynomial
 **************************************************/
void poly_acc(int64_t r[4], const poly *a)
{
  unsigned int i;
  memset(r, 0, 4 * sizeof(int64_t));
  for (i = 0; i < PQMX_L; i++)
  {
    r[0] = barrett_reduce(r[0] + a->coeffs[i * 4 + 0]);
    r[1] = barrett_reduce(r[1] + a->coeffs[i * 4 + 1]);
    r[2] = barrett_reduce(r[2] + a->coeffs[i * 4 + 2]);
    r[3] = barrett_reduce(r[3] + a->coeffs[i * 4 + 3]);
  }
}
 ...
 * Name:        poly_neg
 *
 * Description: Inplace Negate polynomial coefficients 
 *
