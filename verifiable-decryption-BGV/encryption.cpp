...
void CreateStatement(Statement &stmt, const EncKey &encKey){
  ZZ_pE m, finalM; Ciphertext ctx, finalCtx;
  for (int j = 0; j < tau; j++){
    SampleMessage(m); Encrypt(ctx,encKey,m);
    finalM = finalM + m; finalCtx.CTXu = finalCtx.CTXu + ctx.CTXu;
    finalCtx.CTXv = finalCtx.CTXv + ctx.CTXv;
  }
  stmt.messages[0] = finalM; stmt.ciphertexts[0] = finalCtx;}
